...
Modify_config_method(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -m "${ssr_method}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_protocol(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -O "${ssr_protocol}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_obfs(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -o "${ssr_obfs}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_protocol_param(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -G "${ssr_protocol_param}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_speed_limit_per_con(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -s "${ssr_speed_limit_per_con}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_speed_limit_per_user(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -S "${ssr_speed_limit_per_user}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_transfer(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -t "${ssr_transfer}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
Modify_config_forbid(){
    match_edit=$(python2 mujson_mgr.py -e -p "${ssr_port}" -f "${ssr_forbid}"|grep -w "edit user ")
    if [[ -z "${match_edit}" ]]; then
...
debian_View_user_connection_info(){
    ...
    IP_total=`netstat -anp |grep 'ESTABLISHED' |grep 'python2' |grep 'tcp6' |awk '{print $5}' |awk -F ":" '{print $1}' |sort -u |grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" |wc -l`
    user_list_all=""
    for((integer = 1; integer <= ${user_total}; integer++))
    do
        user_port=$(echo "${user_info}"|sed -n "${integer}p"|awk '{print $4}')
        user_IP_1=`netstat -anp |grep 'ESTABLISHED' |grep 'python2' |grep 'tcp6' |grep ":${user_port} " |awk '{print $5}' |awk -F ":" '{print $1}' |sort -u |grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}"`
        if [[ -z ${user_IP_1} ]]; then
            user_IP_total="0"
        else
        ...
        user_info_233=$(python2 mujson_mgr.py -l|grep -w "${user_port}"|awk '{print $2}'|sed 's/\[//g;s/\]//g')
...
centos_View_user_connection_info(){
    ...
    user_total=$(echo "${user_info}"|wc -l)
    [[ -z ${user_info} ]] && echo -e "${Error} 没有发现 用户，请检查 !" && exit 1
    IP_total=`netstat -anp |grep 'ESTABLISHED' |grep 'python2' |grep 'tcp' | grep '::ffff:' |awk '{print $5}' |awk -F ":" '{print $4}' |sort -u |grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" |wc -l`
    user_list_all=""
    for((integer = 1; integer <= ${user_total}; integer++))
    do
        user_port=$(echo "${user_info}"|sed -n "${integer}p"|awk '{print $4}')
        user_IP_1=`netstat -anp |grep 'ESTABLISHED' |grep 'python2' |grep 'tcp' |grep ":${user_port} "|grep '::ffff:' |awk '{print $5}' |awk -F ":" '{print $4}' |sort -u |grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}"`
        if [[ -z ${user_IP_1} ]]; then
            user_IP_total="0"
        else
            user_IP_total=`echo -e "${user_IP_1}"|wc -l`
...
Add_port_user(){
    ...
    if [[ "$lalal" == "install" ]]; then
        ...
        do
            Set_config_all
            match_port=$(python2 mujson_mgr.py -l|grep -w "port ${ssr_port}$")
            [[ ! -z "${match_port}" ]] && echo -e "${Error} 该端口 [${ssr_port}] 已存在，请勿重复添加 !" && exit 1
            match_username=$(python2 mujson_mgr.py -l|grep -w "user \[${ssr_user}]")
...
Del_port_user(){
    ...
    do
        ...
        if [[ ! -z ${del_user} ]]; then
            port=${del_user_port}
            match_del=$(python2 mujson_mgr.py -d -p "${del_user_port}"|grep -w "delete user ")
            if [[ -z "${match_del}" ]]; then
